...
class ReqHdata
  ...
  def self.get_hdata
    if $*.size != 2
      ...
      p "Demo: req_hdata.rb EUR USD"
      p "Demo: req_hdata.rb USD CAD"
      return 
    end
    p 'getting hdata'
    ...
    # contract.m_symbol= jl_str "EUR"
    # contract.m_currency= jl_str "USD"
    contract.m_symbol= jl_str $*.first
    contract.m_currency= jl_str $*.last
    contract.m_secType= jl_str "CASH"
    contract.m_exchange= jl_str "IDEALPRO"
...
File.rename('sysout_1.log',"csv_files/#{$*.first}_#{$*.last}_data_#{tn}")
p "done"
