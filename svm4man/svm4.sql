...
-- Get closing price 4 rows (usually 4 hours) in the future:
...
,CASE WHEN clse4 IS NULL THEN NULL 
      WHEN (clse4 - clse)/clse < -0.0006 THEN 'up'
      ELSE 'nup' END gatt
FROM svm410
ORDER BY ydate
/
...
SELECT MIN(ydate),COUNT(ydate),MAX(ydate)FROM svm410;
...
-- I should see 6 x 4 rows that are too far in the future to give me a gatt value yet:
