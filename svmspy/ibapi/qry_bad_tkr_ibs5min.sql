...
CREATE OR REPLACE VIEW ibs5min20 AS
SELECT
tkr
...
CREATE OR REPLACE VIEW ibs5min20_ld AS
SELECT
tkr
...
SELECT * FROM ibs5min20_ld;

SELECT * FROM ibs5min20 WHERE lag_diff = (SELECT MIN(min_lag_diff)FROM ibs5min20_ld);

SELECT * FROM ibs5min20 WHERE lag_diff = (SELECT MAX(max_lag_diff)FROM ibs5min20_ld);

exit

-- SELECT COUNT(*)FROM ibs5min20 WHERE lag_diff IN(SELECT max_lag_diff FROM ibs5min20_ld);
...
,i.minutes_diff
FROM ibs5min20_ld l, ibs5min20 i
...
-- Now join ibs5min12 with ibs5min20_ld
...
,lead_clse2
,lead_clse3
FROM ibs5min20_ld l, ibs5min12 i
WHERE l.max_lag_diff = i.lag_diff
