...
FROM
  (SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate2<'2007-01-01')a
 ,(SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate1>'2007-01-01')b
...
,COUNT(a.pair)row_cnt
FROM
  (SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate2<'2008-01-01')a
 ,(SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate1>'2008-01-01')b
WHERE a.pair = b.pair AND a.woq = b.woq
GROUP BY a.pair,a.woq,a.day_of_week
-- If the pair count is low, it is noise:
HAVING COUNT(a.pair)> 123
...
,COUNT(a.pair)row_cnt
FROM
  (SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate2<'2010-01-01')a
 ,(SELECT pair,woq,day_of_week,wgain FROM wjw WHERE ydate1>'2010-01-01')b
WHERE a.pair = b.pair AND a.woq = b.woq
GROUP BY a.pair,a.woq,a.day_of_week
-- If the pair count is low, it is noise:
HAVING COUNT(a.pair)> 123
