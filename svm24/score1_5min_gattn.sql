...
,s.sc_corr
FROM modsrc24
,(SELECT AVG(sc_corr)sc_corr FROM modsrc24 WHERE gattn='up'AND 11+ydate>(SELECT MAX(ydate)FROM modsrc24))s
WHERE ydate = '&1'||' '||'&2'
AND pair = '&3'
/
...
SELECT AVG(sc_corr)sc_corr FROM modsrc24 WHERE gattn='up'AND 11+ydate>(SELECT MAX(ydate)FROM modsrc24);
