...
,s.sc_corr
FROM modsrc24
,(SELECT AVG(sc_corr)sc_corr FROM modsrc24 WHERE 11+ydate>'&1'||' '||'&2'AND 1+ydate<'&1'||' '||'&2')s
WHERE ydate = '&1'||' '||'&2'
AND pair = '&3'
/
...
SELECT AVG(sc_corr)sc_corr FROM modsrc24 WHERE 11+ydate>'&1'||' '||'&2'AND 1+ydate<'&1'||' '||'&2'
