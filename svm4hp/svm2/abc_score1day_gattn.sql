...
,chf_g35
,chf_g36
,NVL(s.score,0.2) sscore
FROM abc_ms14 m, fxscores_gattn s
WHERE m.ydate = '&1'||' '||'&2'
AND 'abc'||m.ydate = s.prdate(+)
/


-- rpt
-- We should see just 1 row:
SELECT AVG(sscore),COUNT(prdate) FROM sme;
...
-- rpt
SELECT gattn, AVG(sscore),COUNT(prdate) FROM bme GROUP BY gattn;
SELECT MAX(prdate) FROM bme;

-- Now build model from bme and score sme a 2nd time
@score2gattn.sql
